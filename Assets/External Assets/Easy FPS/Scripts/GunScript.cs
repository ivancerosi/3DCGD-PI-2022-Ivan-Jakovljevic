...
public class GunScript : MonoBehaviour {
	...
	private void ShootMethod(){
		if(waitTillNextFire <= 0 && !reloading && pmS.maxSpeed < 5){
			...
			if(bulletsInTheGun > 0){
				...
				if (bullet)
				{
					Debug.DrawRay(bulletSpawnPlace.transform.position, bulletSpawnPlace.transform.forward*100, Color.red);
	...
	*/
	...
	void DrawCrosshair(){
		...
		if(!ViewModel.Instance.paused && Input.GetAxis("Fire2") == 0){//if not aiming draw
